...
export default function Detail({ navigation, route }) {
  ...
  useEffect(async () => {
    ...
    setIsLoading(true);
    fetch(
      "https://sialbert.000webhostapp.com/teknisi/schedule/show/" + alat.id_alat
    )
  ...
  let calenderItems = objectList.reduce((acc, leave) => {
    ...
    while (end.isAfter(start)) {
      Object.assign(dateRange, {
        [start.format("YYYY-MM-DD")]: { selected: true, color: "#ffcd04" },
      });
  ...
  const photo =
    "https://sialbert.000webhostapp.com/" + alat.foto + "/" + alat.foto;
  return (
