...
export default function DetailOrder({ navigation, route }) {
  ...
  useEffect(async () => {
    ...
    setIsLoading(true);
    const url =
      "http://e565-180-242-214-45.ngrok.io/api/detail-orders/" + id_order;
    fetch(url, {
      method: "GET",
      headers: {
  ...
  useEffect(async () => {
    setIsLoading(true);
    const url =
      "http://e565-180-242-214-45.ngrok.io/api/cekPayments/" + order.id;
    fetch(url, {
      method: "GET",
      headers: {
        //  'Accept': 'application/json',
    ...
    axios
      ...
      .then((response) => {
        ...
        if (success == false) {
          ...
          Alert.alert(
            "Surat Ketetapan Retribusi (SKR) belum terbit!",
            "Lakukan pembayaran jika SKR telah terbit!",
            [
              {
                text: "OK",
                onPress: () => {},
          ...
          if (total_hari > 0) {
            navigation.navigate("Pembayaran", {
              id_order: id_order,
              skr: data.skr,
              dateSkr: data.created_at,
              total_harga: total_harga_perhari,
            });
          } else {
            navigation.navigate("Pembayaran", {
              id_order: id_order,
              skr: data.skr,
              dateSkr: data.created_at,
              total_harga: total_harga_perjam,
            });
          }
        }
      })
            ...
            axios({
              ...
              .then((response) => {
                ...
                if (status == 200) {
                  ...
                  Alert.alert(
                    "Anda tidak dapat melakukan pembatalan.",
                    "Masa penyewaan telah berakhir."
                  );
                  setVisible(false);
                  setIsDisabled(false);
                } else if (message == "Alat Telah Dipakai!") {
                  Alert.alert(
                    "Tidak dapat melakukan pembatalan jika telah memakai alat."
                  );
                  setVisible(false);
                  setIsDisabled(false);
                }
        ...
        <ScrollView style={{ paddingBottom: 8 }}>
          <Card style={{ backgroundColor: "#fff", paddingBottom: 32 }}>
            ...
            >
              ...
              >
                Kode Pemesanan: {order.kode_pesanan}
              </Text>
            </View>
            <View style={{ padding: 16 }}>
              {category_order_id == "1" && (
                <View>
                  {payment.status == "1" && (
                    <Badge style={{ backgroundColor: "#ffcd04" }}>
                      {payment.message}
                    </Badge>
                  )}
                  {payment.status == "2" && (
                  ...
                  {payment.status == "3" && <Badge>{payment.message}</Badge>}
                  {payment.status == "4" && <Badge>{payment.message}</Badge>}
                </View>
              )}
              <Card style={styles.card}>
                ...
                >
                  ...
                  <Text>
                    {Moment(dtMulai).format("dddd, DD MMMM YYYY HH:mm")}
                  </Text>
                </View>
                <View
                  style={{
                ...
                >
                  ...
                  <Text>
                    {Moment(dtSelesai).format("dddd, DD MMMM YYYY HH:mm")}
                  </Text>
                </View>
                {order.total_hari > 0 ? (
                  <View
              ...
              <Card style={styles.card2}>
                ...
                {detail.map((item) => (
                  <Card
                    key={item?.[0].detail_order.id}
                    {...item}
                    style={styles.card3}
                  >
                    {(() => {
                      const harga_sewa_perhari =
                        item.alat?.[0].harga_sewa_perhari;
                            ...
                            >
                              <View style={{ flexDirection: "row" }}>
                                {category_order_id == "1" && (
                                  ...
                                  >
                                    Rp.
                                    {Number(item?.[0].alat.harga_sewa_perhari)
                                      .toFixed(0)
                                      .replace(
                                        /(\d)(?=(\d{3})+(?!\d))/g,
                                        "$1."
                              ...
                              {category_order_id == "1" && (
                                <Text style={{ margin: 16 }}>
                                  Rp.
                                  {Number(
                                    item?.[0].alat.harga_sewa_perhari *
                                      total_hari
                                  )
                                    .toFixed(0)
                                    .replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.")}
                            ...
                            >
                              <View style={{ flexDirection: "row" }}>
                                {category_order_id == "1" && (
                                  ...
                                  >
                                    Rp.
                                    {Number(item?.[0].alat.harga_sewa_perjam)
                                      .toFixed(0)
                                      .replace(
                                        /(\d)(?=(\d{3})+(?!\d))/g,
                                        "$1."
                              ...
                              {category_order_id == "1" && (
                                <Text style={{ margin: 16 }}>
                                  Rp.
                                  {Number(
                                    item?.[0].alat.harga_sewa_perjam * total_jam
                                  )
                                    .toFixed(0)
                                    .replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.")}
                                  ,-
